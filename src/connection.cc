...
const Peers& Connection::peers() const {
  return peers_;
}
...
void Connection::Connect(const std::string& server, int port,
  ...
  if (server_address_.IsUnresolvedIP()) {
    ...
    resolver_->SignalDone.connect(this, &Connection::OnResolveResult);
...
void Connection::OnResolveResult(
...
bool Connection::SendToPeer(int peer_id, const std::string& message) {
  if (state_ != CONNECTED)
    return false;
...
bool Connection::SendHangUp(int peer_id) {
...
bool Connection::ConnectControlSocket() {
  ASSERT(control_socket_->GetState() == rtc::Socket::CS_CLOSED);
...
void Connection::OnConnect(rtc::AsyncSocket* socket) {
...
void Connection::OnHangingGetConnect(rtc::AsyncSocket* socket) {
  char buffer[1024];
  sprintfn(buffer, sizeof(buffer),
...
bool Connection::ReadIntoBuffer(rtc::AsyncSocket* socket,
                                          std::string* data,
...
void Connection::OnHangingGetRead(rtc::AsyncSocket* socket) {
...
void Connection::OnMessage(rtc::Message* msg) {
